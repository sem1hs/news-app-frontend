...
type FixtureTableProps =
    | {
        grouped: true;
        data: Record<string, FixtureResponse[]>;
    }
    | {
        grouped?: false;
...
export default function FixtureTable(props: FixtureTableProps) {
    ...
    const renderTable = (fixtures: FixtureResponse[]) => (
                        ...
                        <tr key={fixture.id}>
                            ...
                            <td className="p-2 border border-amber-500 text-center">
                                <div className="flex gap-2 justify-center">
                                    <button onClick={() => handleUpdate(fixture)} className="rounded bg-amber-500 px-3 py-1 text-xs font-semibold text-black hover:bg-amber-400 transition">
                                        Düzenle
                                    </button>

                                    <button onClick={() => handleDelete(fixture.id)} className="rounded bg-red-500 px-3 py-1 text-xs font-semibold text-white hover:bg-red-400">
                                        Sil
                                    </button>
                                </div>
                            </td>
    ...
    return (
        <div className="mt-6 px-6 flex flex-col gap-8">
            {props.grouped
                ? Object.entries(props.data).map(([groupKey, fixtures]) => (
                    <div key={groupKey}>
