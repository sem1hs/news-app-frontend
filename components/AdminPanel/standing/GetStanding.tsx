...
const GetStanding = () => {
  ...
  return (
    <div className="mt-6 px-6">
      ...
      {visible && standings && standings.length !== 0 && (
        <div className="mt-4 max-h-[500px] overflow-y-auto custom-scrollbar">
          <StandingTable data={standings} />
        </div>
      )}
      {visible && (!standings || standings.length === 0) && (
        <EmptyState
          title="Bu lig için puan durumu tanımlanmamış"
          description="Seçilen lig için henüz puan durumu eklenmemiş."
        />
      )}
    </div>
  );
