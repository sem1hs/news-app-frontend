...
const GetFixtureByLeague = () => {
  ...
  const groupedData = useMemo(() => {
    if (!fixture) return {};
    return fixture.reduce<Record<string, FixtureResponse[]>>((acc, item) => {
  ...
  return (
    <div className="mt-6 px-6">
      ...
      {visible && Object.keys(groupedData).length !== 0 && (
        <div className="mt-4 max-h-[500px] overflow-y-auto custom-scrollbar">
          <FixtureTable grouped data={groupedData} />
        </div>
      )}
      {visible && Object.keys(groupedData).length === 0 && (
        <EmptyState
          title="Bu lig için fikstür tanımlanmamış"
          description="Seçilen lig için henüz fikstür eklenmemiş."
        />
      )}
    </div>
  );
